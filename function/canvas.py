...
def ids_login(qq: int) -> Session:
    ...
    s = requests.session()
    ss = s.post("https://ids.tongji.edu.cn:8443/nidp/app/login?id=Login&sid=0&option=credential&sid=0").text
    # print(ss)
    secretId = ss.split('<input type="hidden" value="')[1].split('" id="secretId">')[0]
    print(secretId)
    Ecom_Password = sm_2encrypt(Ecom_Password, secretId)
    while 1:
        try:
            code = crack_main(s)
            break
    ...
    print(data)
    res = s.post("https://ids.tongji.edu.cn:8443/nidp/app/login", data=data)
    print(res.text)
    return s
...
def courses_login(s: Session, qq: int) -> Session:
    """courses登录"""
    ss = s.get(
        "https://ids.tongji.edu.cn:8443/nidp/oauth/nam/authz?scope=profile&response_type=code&redirect_uri=https%3A%2F%2Fcourses.tongji.edu.cn%2Fsign-in&client_id=241129f4-7528-4207-8751-ee240727b41c"
    )
...
async def timetable(app, s, group, member, flag=True):
    url = (
        "http://canvas.tongji.edu.cn/api/v1/planner/items?per_page=50&start_date="
    ...
    for i in data:
        ...
        date = datetime.datetime.strptime(
            i["plannable_date"], "%Y-%m-%dT%H:%M:%SZ"
        ...
        if i["context_type"] == "User":
            if (
                flag
                and i["planner_override"] != None
                and i["planner_override"]["marked_complete"] != False
            ):
                continue
            ss += (
                "\n标题："
            ...
            if flag and "submissions" in i:
                sub = i["submissions"]
                if (type(sub) == bool and sub == True) or (
                    type(sub) == dict and sub["submitted"] == True
                ):
                    continue
            sstr += (
                "\n标题："
...
async def addddl(app, s, group, member, msg: str):
    ...
    if event_start == "":
        ...
        if len(subtext) == 3:
            ...
            return
        event_end = event_start = (
            datetime.datetime(int(y), int(m), int(d)) - datetime.timedelta(hours=8)
        ).strftime("%Y-%m-%dT%H:%M:%SZ")

    url = "http://canvas.tongji.edu.cn/api/v1/calendar_events"

    data = {
        "calendar_event[title]": event_title,
        "calendar_event[start_at]": event_start,
        "calendar_event[end_at]": event_end,
        "calendar_event[location_name]": event_local,
        "calendar_event[context_code]": "user_" + str(get_id(member.id)),
        "_method": _method,
        "authenticity_token": parse.unquote(
...
async def markfinish(app, s: Session, group, member, msg: str):
    ...
    if event_id == None:
        ...
        url = "http://canvas.tongji.edu.cn/api/v1/planner/overrides/" + str(event_id)
...
async def courses_calender(app, s, group, member):
    ...
    data = {"user_token": token, "start": date, "end": date}
    c = s.post("https://courses.tongji.edu.cn/tmbs/api/v1/user/calendar/my", data=data)
    cc = c.json()["data"]
...
async def canvas(app, group, member, msg, s: Session):
    try:
        ...
        if str(e) == "验证码错误":
            await app.send_group_message(group, MessageChain([Plain("登录超时，请稍后再试。")]))
        return
    if msg == "canvas.todo":
